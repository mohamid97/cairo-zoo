...
@section('content')
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>{{ __('main.parteners') }}</h1> <!-- Translated title -->
                </div>
                <div class="col-sm-6">
    ...
    <section class="content">
        <div class="container-fluid">
            <div>
                <a href="{{ route('admin.parteners.add') }}" style="color: #FFF">
                    <button class="btn btn-info">
                        <i class="nav-icon fas fa-plus"></i> {{ __('main.add_new_partener') }} <!-- Translated button -->
                    </button>
                </a>
            </div>
            <br>
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    <!-- Search Form -->
                    <form method="GET" action="{{ route('admin.parteners.index') }}" class="input-group">
            ...
            <div class="card card-info">
                <div class="card-header">
                    <h3 class="card-title">{{ __('main.parteners') }}</h3> <!-- Translated title -->
                </div>
                <!-- /.card-header -->
                <div class="card-body">
                    <table class="table table-bordered">
                        ...
                        <tbody>
                        @forelse($parteners as $index => $partener)
                            <tr>
                                <td>{{ $index + 1 }}</td>
                                <td>
                                    <img src="{{ asset('uploads/images/parteners/' . $partener->icon) }}" width="70px" height="70px">
                                </td>
                                <td>{{ $partener->name }}</td>
                                <td>{{ $partener->address }}</td>
                                <td>
                                    <a href="{{ route('admin.parteners.edit', ['id' => $partener->id]) }}">
                                        <button class="btn btn-sm btn-info"><i class="nav-icon fas fa-edit"></i> {{ __('main.edit') }}</button> <!-- Translated edit button -->
                                    </a>

                                    @if($partener->deleted_at == null)
                                        <a href="{{ route('admin.parteners.soft_delete', ['id' => $partener->id]) }}">
                                    ...
                                    @else
                                        <a href="{{ route('admin.parteners.restore', ['id' => $partener->id]) }}">
                                    ...
                                    <a href="{{ route('admin.parteners.destroy', ['id' => $partener->id]) }}">
